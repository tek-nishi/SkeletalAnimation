...
void updateNodeDerivedMatrix(const std::shared_ptr<Node>& node,
  ...
  node->derived_matrix = parent_matrix * node->matrix;
  node->invert_matrix  = node->derived_matrix.inverted();

  for (auto child : node->children) {
    updateNodeDerivedMatrix(child, node->derived_matrix);
  }
}
  ...
  for (auto& node : model.node_list) {
    for (auto& mesh : node->mesh) {
      ...
      for (auto& bone : mesh.bones) {
        ...
        bone_matrix.push_back(node->invert_matrix * local_node->derived_matrix * bone.offset);
      }

	  mesh.body = mesh.weighted;
